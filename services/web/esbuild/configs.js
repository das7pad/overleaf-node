...
const CONFIGS = [
  ...
  {
    DESCRIPTION: 'marketing bundles',

    metafile: true,
    splitting: true,
    format: 'esm',
    entryPoints: [
      Path.join(FRONTEND_PATH, 'js/marketing.js'),
      ...glob.sync(Path.join(FRONTEND_PATH, 'js/pages/**/*.js')),
      ...glob.sync(Path.join(MODULES_PATH, '*/frontend/js/pages/**/*.js')),
    ],
    outbase: Path.join(FRONTEND_PATH, 'js'),
    outdir: Path.join(PUBLIC_PATH, 'js'),
    inject: [Path.join(ROOT, 'esbuild/inject/bootstrapMarketing.js')],
    define: {
      'process.env.NODE_ENV': '"production"',
    },
  },
